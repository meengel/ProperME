...
class ProperME:
    def __init__(self, dependingdict, computegraphname="_computegraph", active=True):
        ...
        self._has_children = self._get_has_childrendict(self._depending) # determine whether any other method depends on key-method
        ...
        if self.active:
            ...
            if "get"==mode: # for getters
                def getter(*args,**kwargs):
                    _computegraph = self._get_computegraph(args[0]) # load computegraph from obj -> important for pickle
                ...
                def setter(*args,**kwargs):
                    ...
                    _computegraph["get"+varname] = False # mark corresponding get-method as done as being set by setter
    ...
    @staticmethod
    def _get_has_childrendict(dependingdict):
        has_children_dict = {}
    ...
    def _get_dependentdict(self, key, dependentlist, visited):
        ...
        for key_ in dependentlist:
            if self._has_children[key_] and not visited[key_]:
                self._get_dependentdict(key_, dependentlist, visited)
    
    def _get_computegraph(self, obj):
